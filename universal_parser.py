...
class Field:
    def __init__(self, name, data_type, selector=None, separator=None):
        ...
        self.separator = separator  # новый параметр для разделителей
        ...
        try:
            ...
            if 'fields' in template:
                for field_name, field_config in template['fields'].items():
                    ...
                    separator = field_config.get('separator', None)
                    field = Field(field_name, data_type, selector, separator)
                    widget = FieldWidget(field, self)
                    widget.selector_edit.setText(selector)
                    if separator and data_type == 'list':
                        # Установить соответствующий разделитель в комбобокс
                        separator_map_reverse = {
                            ...
                            '. ': 'Точка (.)',
                            '\n': 'Перенос строки',
                            'numbered': 'Нумерация (1., 2.)',
                            'cyrillic': 'Буквы (а., б.)',
                            'latin': 'Буквы (a., b.)'
                        }
                        if separator in separator_map_reverse:
                            index = widget.separator_combo.findText(separator_map_reverse[separator])
                            if index >= 0:
                                widget.separator_combo.setCurrentIndex(index)
                    self.fields_layout.insertWidget(self.fields_layout.count() - 1, widget)
                    self.fields.append(field)
